                ...
                <div class="header-top-right">
                    <div class="account">
                        <a href="{{ route('frontend.wishlist.index') }}" class="position-relative wishlist-header-link" style="margin-right: 15px; {{ count(array_filter(session('wishlist', []))) > 0 ? '' : 'display: none;' }}">
                            ...
                            <span id="wishlist-counter-home" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size: 10px; padding: 3px 5px; border: 1px solid #fff; margin-top: 5px;">
        ...
        <nav class="navbar navbar-expand-lg">
            <div class="container">
                <a class="navbar-brand" href="{{ route('frontend.home') }}">
                    @if ($logo)
                        <img src="{{ $logo }}" class="logo-display" alt="{{ $displayValue($settings->company_name ?? null, 'Logo') }}">
                    @endif
                    @if ($logoDark)
                        <img src="{{ $logoDark }}" class="logo-scrolled" alt="{{ $displayValue($settings->company_name ?? null, 'Logo') }}">
                    @endif
                </a>
